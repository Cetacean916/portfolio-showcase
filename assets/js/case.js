(function () {
  ...
  if (project.id === "pf07" && project.refinement) {
    ...
    const baselineEvidenceAction = language === "en" ? "Open earlier baseline evidence ↗" : "기존 기준선 근거 열기 ↗";
    const finalProofCards = copy.finalProofLabels.map((label, index) => `<article class="pf07-proof"><span>${String(index + 1).padStart(2, "0")}</span><h3>${escape(label)}</h3><p>${escape(finalStateDetails[index])}</p><a href="${escape(`${project.refinement.mediaBase}/${project.refinement.postCandidateAssets[index]}`)}" target="_blank" rel="noopener" data-historical-release-evidence>${escape(baselineEvidenceAction)}</a></article>`).join("");
    ...
    root.innerHTML = `<article class="pf07-case case-page" data-pf07-case>
      ...
      ${videoSections}
      <section class="pf07-section"><div class="pf07-section-heading"><div><p class="eyebrow">DELIVERY OPTIONS</p><h2>${escape(copy.packageTitle)}</h2></div><p>${escape(copy.packageIntro)}</p></div><aside class="pf07-candidate-state" data-candidate-release-boundary><b>${language === "en" ? "CURRENT OFFSET CANDIDATE" : "현재 OFFSET 후보"}</b><code>${escape(project.refinement.currentCandidate.buildId)}</code><span>${escape(project.refinement.currentCandidate.publicationState)}</span></aside><div class="pf07-package-grid">${cards(copy.packages, "pf07-package")}</div><div class="pf07-download-grid" aria-label="${escape(copy.downloadAction)}">${releaseDownloads}</div></section>
      <section class="pf07-section pf07-final"><div class="pf07-section-heading"><div><p class="eyebrow">PUBLISHED BASELINE</p><h2>${escape(copy.finalProofTitle)}</h2></div><p>${escape(copy.finalProofIntro)}</p></div><div class="pf07-proof-grid">${finalProofCards}</div></section>
