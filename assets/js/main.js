(function () {
  ...
  const cards = orderedProjects.map((project, index) => {
    ...
    const secondaryAction = project.refinement?.releaseUrl
      ? `<a class="text-link" href="${escape(project.refinement.releaseUrl)}" target="_blank" rel="noopener">1.0.2 릴리즈</a>`
      : project.liveUrl
        ...
        : project.demo
          ...
          : project.video
            ...
            : "";
    const releaseLabel = featured ? "LATEST · 1.0.2" : `INDEX · ${String(index + 1).padStart(2, "0")}`;
